...
function renderMiniWaveform(canvas, waveformData) {
  ...
  var POINTS = 60;
  var gap = 1;
  var barW = Math.max(1, (W - gap * (POINTS - 1)) / POINTS);
  var step = Math.max(1, Math.floor(waveformData.length / POINTS));
  ctx.clearRect(0, 0, W, H);
  for (var i = 0; i < POINTS; i++) {
    var segStart = i * step;
    var segEnd = Math.min((i + 1) * step, waveformData.length);
    var maxAmp = 0;
    for (var j = segStart; j < segEnd; j++) {
      maxAmp = Math.max(maxAmp, Number(waveformData[j]) || 0);
    }
    var amp = Math.max(0.02, maxAmp);
...
window.renderMiniWaveform = renderMiniWaveform;
