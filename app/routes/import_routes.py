        ...
        with _ts_lock:
            ...
            for track in tracks:
                ...
                if track.file_path in unchanged_paths:
                    continue
                # Restore cached analysis BEFORE persisting to SQLite so the
                # write captures the restored state. (A plain in-memory dict
                # store mutates the same object in-place and sees the update
                # on the next read; SQLite deserialises a fresh Track on every
                # read, so post-persist mutations are silently lost.)
                if restore(track):
                    restored_count += 1
